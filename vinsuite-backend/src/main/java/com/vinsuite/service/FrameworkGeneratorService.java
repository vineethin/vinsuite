...
public class FrameworkGeneratorService {
    ...
    public byte[] generateFramework(FrameworkConfigRequest config) throws IOException {
        ...
        System.out.println("➡️ Requested language: " + config.getLanguage());

        if ("java".equalsIgnoreCase(config.getLanguage())) {
            generateJavaFrameworkTemp(baseDir, config);
    ...
    private void generateJavaFrameworkTemp(Path baseDir, FrameworkConfigRequest config) throws IOException {
        System.out.println("🟡 Using NEWLY RENAMED METHOD generateJavaFrameworkTemp");
        ...
        Files.writeString(baseDir.resolve("pom.xml"), getPomXml());
    ...
    private void generateCsharpFramework(Path baseDir, FrameworkConfigRequest config) throws IOException {
        ...
        Files.writeString(baseDir.resolve("Tests/SampleTest.cs"), "using NUnit.Framework;\n[TestFixture]\npublic class SampleTest {\n  [Test] public void Example() => Assert.Pass();\n}");
        ...
        Files.writeString(baseDir.resolve("Config/config.json"), "{ \"browser\": \"chrome\", \"url\": \"https://example.com\" }");
    }

    private String getPomXml() {
        return """
